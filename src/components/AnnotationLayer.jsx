/** Renders one enriched annotation (line/area/point/concern/text) as SVG. Shared
 *  by the workspace canvas and the report pages so both stay visually identical. */
export default function AnnotationLayer({ annotations, showBreachNumbers, markerId }) {
  const concernMarkerId = `${markerId}-concern`
  return (
    <>
      <defs>
        ...
        <marker id={concernMarkerId} markerWidth="4" markerHeight="4" refX="3.2" refY="1.6" orient="auto" markerUnits="strokeWidth">
          <path d="M0,0 L3.2,1.6 L0,3.2 Z" style={{ fill: '#0891b2', fillOpacity: 0.8 }} />
        </marker>
      </defs>
      {annotations.map((ann) => (
        <g key={ann.id}>
          ...
          {ann.isMarker && (
            <>
              <line x1={ann.tailX} y1={ann.tailY} x2={ann.labelX} y2={ann.labelY} style={{ stroke: ann.color, strokeWidth: 1.2, strokeOpacity: 0.8 }} markerEnd={`url(#${ann.isConcern ? concernMarkerId : markerId})`} />
              ...
              {showBreachNumbers && (
                <foreignObject x={ann.tailX} y={ann.tailY} width="1" height="1" style={{ overflow: 'visible', pointerEvents: 'none' }}>
                  <div style={{ ...labelStyle('#000', 15, 700, true), borderRadius: 10 }}>{ann.markerLabel}</div>
                </foreignObject>
              )}
            </>
          )}
...
function labelStyle(color, fontSize, weight = 600, transparent = false) {
  return {
    ...
    padding: '1px 6px',
    borderRadius: 3,
    boxShadow: transparent ? 'none' : '0 0 0 1px rgba(15,23,42,0.08)',
